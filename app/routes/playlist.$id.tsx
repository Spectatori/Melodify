...
export const action = async ({ request }: ActionFunctionArgs): Promise<SpotifyActionResponse | FineTuneResponse> => {
  ...
  if (actionType === 'removeSongs') {
    const playlistId = formData.get('playlistId')?.toString();
    const songsToRemoveIndices = formData.get('songsToRemoveIndices')?.toString();
    
    if (!playlistId || !songsToRemoveIndices) {
      return { success: false, error: 'Missing required data for removing songs' };
    }
    
    try {
      const indicesToRemove = JSON.parse(songsToRemoveIndices) as number[];
      console.log(`Removing ${indicesToRemove.length} songs from playlist`);
      
      // Get current playlist
      const currentPlaylist = getPlaylist(playlistId);
      if (!currentPlaylist) {
        return { success: false, error: 'Playlist not found' };
      }
      
      // Filter out the songs at the specified indices
      const updatedSongs = currentPlaylist.songs.filter((_, index) => !indicesToRemove.includes(index));
      const updatedSongDetails = ((currentPlaylist as any).songDetails || []).filter((_: any, index: number) => !indicesToRemove.includes(index));
      
      // Make sure we have at least some songs left
      if (updatedSongs.length === 0) {
        return { success: false, error: 'Cannot remove all songs from playlist' };
      }
      ...
      console.log(`Successfully removed ${indicesToRemove.length} songs, ${updatedSongs.length} songs remaining`);
...
// Updated PlaylistInfo Props Interface
interface PlaylistInfoProps {
  ...
  onRemoveSelectedSongs: () => void; // NEW PROP
...
// Updated PlaylistInfo Component
...
export default function PlaylistPage() {
  ...
  useEffect(() => {
    if (fetcher.state === 'idle' && fetcher.data) {
      ...
      if (fetcher.data.success) {
        if (currentActionType === 'removeSongs' && fetcher.data.playlist) {
          // Update the playlist with removed songs
          setPlaylist(fetcher.data.playlist);
          setSelectedSongs(new Set()); // Clear selection
          setSuccessMessage(`Successfully removed ${fetcher.data.removedCount || selectedSongs.size} songs!`);
          setErrorMessage(undefined);
        } else if (fetcher.data.newSongs) {
  ...
  const handleRemoveSelectedSongs = () => {
    if (selectedSongs.size === 0) {
      alert("Please select some songs to remove");
      return;
    }
    
    if (!confirm(`Are you sure you want to remove ${selectedSongs.size} selected songs?`)) {
      return;
    }
    
    setIsLoading(true);
    setCurrentActionType('removeSongs');
    
    const indicesToRemove = Array.from(selectedSongs);
    const formData = new FormData();
    formData.append('actionType', 'removeSongs');
    formData.append('playlistId', playlist.id);
    formData.append('songsToRemoveIndices', JSON.stringify(indicesToRemove));
      ...
      <div className='flex w-full px-10 pt-10 pb-20 flex-col'>
        ...
        <div className='flex flex-col lg:flex-row gap-8 h-full'>
          <UpdatedPlaylistInfo
            ...
            onRemoveSelectedSongs={handleRemoveSelectedSongs}
